...
export const en = {
  ...
  creating: 'Creating\u2026',
  dualTitle: 'Dual-model architecture',
  dualIntro: 'This deployment uses two agents: the orchestrator handles your conversation and the worker implements the code. Their authentication is independent: the orchestrator accepts Auth or an API key, while the worker uses the native Codex Auth or API-key configuration.',
  orchestratorModel: 'Orchestrator model',
  orchestratorHint: 'The model your conversation talks to: analysis, task dispatch, and result verification.',
  orchestratorCatalogFailed: 'Loading the orchestrator model catalog failed',
  executorModel: 'Worker agent (Codex)',
  executorHint: 'Authentication and model selection stay in the native Codex configuration. This page does not provide manual worker-model controls.',
  executorEffort: 'Reasoning effort',
  dualSaved: 'Saved.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  creating: '创建中\u2026',
  dualTitle: '双模型架构',
  dualIntro: '本部署由两个 agent 分工协作：编排 agent 负责对话与任务派发，工作 agent 负责代码实现。两者认证彼此独立：编排 agent 支持 Auth 或 API Key，工作 agent 使用 Codex 原生 Auth 或 API Key 配置。',
  orchestratorModel: '编排模型',
  orchestratorHint: '对接用户对话的模型：需求分析、任务派发与产物核验。',
  orchestratorCatalogFailed: '加载编排模型目录失败',
  executorModel: '工作 agent（Codex）',
  executorHint: '认证方式和模型选择由 Codex 原生配置管理，本页面不提供工作 agent 的手动模型调节。',
  executorEffort: '推理力度',
  dualSaved: '已保存。',
