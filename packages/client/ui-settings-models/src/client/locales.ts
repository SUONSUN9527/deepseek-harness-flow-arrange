...
export const en = {
  ...
  dualIntro: 'This deployment splits the work between two models: the orchestrator handles your conversation, the executor implements the code.',
  ...
  executorModel: 'Executor model (Codex)',
  executorHint: 'The model the Codex executor runs on; every delegated implementation task uses it.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  creating: '创建中\u2026',
  dualTitle: '双模型架构',
  dualIntro: '本部署由两个模型分工协作：编排模型对接你的对话，执行模型完成代码实现。',
  orchestratorModel: '编排模型',
  orchestratorHint: '对接用户对话的模型：需求分析、任务派发与产物核验。',
  orchestratorCatalogFailed: '加载编排模型目录失败',
  executorModel: '执行模型（Codex）',
  executorHint: 'Codex 执行代理使用的模型：所有派发的实现类任务均由它完成。',
  executorEffort: '推理力度',
  dualSaved: '已保存。',
