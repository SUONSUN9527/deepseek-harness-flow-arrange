...
- `get(id, caller?)` and `list(caller?)` return non-consuming snapshots. Listing includes only caller-owned and unowned jobs. Snapshots expose `phase`, `revision`, and progress timestamps so clients can distinguish accepted, provisioning, running, terminal, timeout, and orphaned work.
- `read(id, caller?)` consumes the single cursor for stream jobs and reads terminal output idempotently for final-output jobs.
- `kill(id, caller?, reason?)` invokes producer cancellation before changing status. A cancellation throw leaves the job running; success changes it to `stopping` and marks terminal delivery reported. Implementations bound the stopping phase and surface `orphaned` when work cannot be confirmed stopped.
