...
参照 Codex 桌面版形态实现的本地桌面壳：双击启动，选择工作区，应用会在本机拉起 `dsh web` 服务，并把会话界面内嵌到桌面窗口中使用。全部代码独立在本目录，不参与仓库的 pnpm workspace 构建。
...
2. 在启动页选择工作区目录（或点左侧「最近工作区」一键启动），点「启动会话」。
...
| 内嵌页面的浏览器侧状态（localStorage 等） | Electron `userData` | 按「来源(含端口)」隔离，因此默认使用固定端口 **26100**，被占用时才退回随机端口 |
